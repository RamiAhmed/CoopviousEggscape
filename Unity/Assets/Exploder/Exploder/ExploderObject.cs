...
namespace Exploder
{
    ...
    private float isolatingIslandsTime = 0.0f;
        ...
        private void Awake()
        {
            ...
            FragmentPool.Instance.SetFragmentPhysicsOptions(FragmentOptions, use2d);
...
//            }
            ...
            while (cutting)
            {
                ...
                foreach (var mesh in meshSet)
                {
                    if (levelCount[mesh.level] > 0)
                    {
                        ...
                        cutter.Cut(mesh.mesh, mesh.transform, plane, triangulateHoles, AllowOpenMeshCutting, ref meshes, crossSectionVertexColour, crossSectionUV);
        ...
        postProcessingFrames++;
            ...
            while (poolIdx < count)
            {
                ...
                if (FragmentOptions.FragmentMaterial != null)
                {
                    fragment.meshRenderer.sharedMaterial = FragmentOptions.FragmentMaterial;
                }
                else
                {
